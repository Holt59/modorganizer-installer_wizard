...
class WizardInstallerCompletePage(QtWidgets.QWidget):
    ...
    def _displayTweaksObscript(self, file: str, tweaks: List[WizardINISetting]):
        # Assume everything is "set", "setGS" or "SetNumericGameSetting":
        edit = self.ui.tweaksTextEdit

        edit.appendPlainText(f"; Generated by Mod Organizer 2 for '{file}' via Wizard")

        lines = []
        for tweak in tweaks:
            if not isinstance(tweak, WizardINISettingEdit):
                print(f"Cannot disable INI Tweaks in '{file}'.")
                continue

            if tweak.section not in ("set", "setGS", "SetNumericGameSetting"):
                print(f"Incorrect INI Tweak found for '{file}'.")
                continue
    ...
    def _displayTweaksIni(self, file: str, tweaks: List[WizardINISetting]):
        edit = self.ui.tweaksTextEdit
    ...
    def onCurrentItemChanged(
        ...
        tweaks = current.data(Qt.UserRole)
        modified: List[WizardINISettingEdit] = tweaks[0]
        disabled: List[WizardINISetting] = tweaks[1]

        alltweaks: List[WizardINISetting] = modified + disabled  # type: ignore

        if alltweaks[0].section in ("set", "setGS", "SetNumericGameSetting"):
            self._displayTweaksObscript(current.text(), alltweaks)
        else:
            self._displayTweaksIni(current.text(), alltweaks)
